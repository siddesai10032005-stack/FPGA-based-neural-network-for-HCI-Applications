...
Build a lightweight binocular gaze-estimation neural network.

Train and evaluate it using the U2Eyes synthetic eye-tracking dataset.

Predict continuous gaze angles as well as discrete horizontal, vertical, and 9-class gaze directions.

Validate the trained model using a real webcam.

Calibrate the webcam-domain predictions using a 9-point calibration process.

Reduce visible frame-to-frame jitter using temporal smoothing.

Quantize the trained model to INT8 using AMD/Xilinx Vitis AI.

Generate an FPGA-oriented XModel.

Verify that the deployment-safe graph preserves the original FP32 V5 model behavior before hardware execution.
...
Head-pose-aware dataset split

Lightweight V5 binocular CNN

Multi-task gaze regression/classification
